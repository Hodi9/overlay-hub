...
function Get-OcrEngineForRun {
  ...
  Write-Warning "English OCR pack not found - falling back to $($engine.RecognizerLanguage.DisplayName). Recognition of English text may be less accurate."
  return $engine
}

function Read-TrackerConfig {
  if (-not (Test-Path -LiteralPath $ConfigPath)) {
    throw "Missing config: $ConfigPath. Copy the .example.json next to it, fill it in, and rename it."
  }
  $config = Get-Content -LiteralPath $ConfigPath -Raw | ConvertFrom-Json
  if (-not $config.panelUrl -or -not $config.apiKey -or -not $config.gamePath) {
    throw "panelUrl, apiKey and gamePath are required in $ConfigPath."
  }
  return $config
}
...
function Get-CaptureRegion([object]$Config) {
  ...
  # Default: a centered band where most games show chapter/mission title cards.
...
function Send-TrackerText([object]$Config, [string]$Text) {
  $uri = "$($Config.panelUrl.TrimEnd('/'))/$($Config.gamePath)/api/tracker"
  if ($Config.profile) {
...
if ($Calibrate) {
  ...
  $g.Dispose()
  $outPath = Join-Path (Split-Path -Parent $ConfigPath) "calibration-screenshot.png"
  $bmp.Save($outPath, [System.Drawing.Imaging.ImageFormat]::Png)
  $bmp.Dispose()
  Write-Host "Screen resolution: $($screen.Width)x$($screen.Height)"
  Write-Host "Saved a full screenshot to: $outPath"
  Write-Host "Open it, find the pixel box around where the game shows chapter/mission title cards, then set 'region': { x, y, width, height } in your config."
  exit 0
}
...
Write-Host "OCR tracker for '$($config.gamePath)' watching region: x=$($region.X) y=$($region.Y) w=$($region.Width) h=$($region.Height) using $($ocrEngine.RecognizerLanguage.DisplayName) OCR (profile: $profileLabel)"
...
function Tick {
  ...
  try {
    ...
    if ($result.matched) {
      ...
      Write-Host "[$now] Matched: '$text' -> $($ch.day) ($($result.progress.completed)/$($result.progress.total))"
    } else {
