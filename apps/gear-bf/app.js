...
const DEFAULT_CAMPAIGN_LABEL = "Black Friday";
...
async function cutoutProductImage(imageUrl) {
  ...
  try {
    ...
    console.error("gear-bf: cutout failed, falling back to original image", err);
    return imageUrl;
  }
}
...
export function createGearBfApp() {
  const router = express.Router();
  const controlPassword = process.env.GEAR_BF_CONTROL_PASSWORD || "";

  const state = {
    outroEnabled: false,
    outroText: DEFAULT_OUTRO_TEXT,
    overlayScale: 1,
    campaignLabel: DEFAULT_CAMPAIGN_LABEL,
    products: DEFAULT_PRODUCTS.map((p) => ({ id: crypto.randomUUID(), ...p })),
  };

  function authorized(req) {
    if (!controlPassword) return true;
    const key = String(req.headers["x-gear-bf-key"] || "");
  ...
  router.post("/api/config", (req, res) => {
    ...
    if (typeof body.campaignLabel === "string" && body.campaignLabel.trim()) {
      state.campaignLabel = body.campaignLabel.trim();
    }
    res.json({
      outroEnabled: state.outroEnabled,
      outroText: state.outroText,
  ...
  if (!controlPassword) {
    console.log("ADVARSEL: GEAR_BF_CONTROL_PASSWORD er ikke sat — gear-bf-panelet er ubeskyttet for alle med linket.");
  }
